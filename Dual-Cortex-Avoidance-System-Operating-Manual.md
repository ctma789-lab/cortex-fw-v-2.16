...
| `afloor arm` / `disarm` | Active-descent floor braking (armed **by default at boot** since v2.1.7 — `disarm` is a manual override, `arm` re-enables it if you'd disarmed it) |
...
**Comm Cortex: v2.1.6 → v2.1.7.** One change, tagged `[v2.1.7 I-006]` in the source, matching your existing `[v2.1.x I-00x]` convention:

- **`g_activeFloorArmed` default** (~line 692): changed from `false` to `true`. Active floor climb-assist is now armed automatically at every boot — no BLE command needed every flight. This works because `activeOk` already required `!landing` to engage, so defaulting to armed and letting landing mode suppress it naturally gives you "armed whenever not in landing mode" without adding any new logic or flash persistence. `afloor disarm` over BLE still works if you ever want to kill it entirely (e.g. bench testing) — it's a manual override on top of the new default, not removed.
- Note: this only touched `afloor`. `ahoriz` (active horizontal push-away) still defaults to disarmed at boot, per your answer — arm it over BLE each flight if you want it.
- Version bumped: header comment (line 3) and boot banner now read `v2.1.7`. The pre-existing stale `v2.1.4` string later in `setup()` (flagged in §10) was left untouched since it wasn't part of this request — still there if you want it fixed too.
...
**Firmware files provided alongside this doc:** `sensory_cortex_v2_0_5.ino` includes the OF-calibration fix described in §8.2/§9. `comm_cortex_v2_1_7.ino` includes the default-armed floor fix described in §9 — confirm via the `v2.1.7` boot banner. Both should be flashed to their respective boards as a matched pair.
