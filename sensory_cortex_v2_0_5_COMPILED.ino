/*
 * ================================================================
 *  DUAL CORTEX DRONE — SENSORY CORTEX FIRMWARE v2.0.5
 * ================================================================
...
// Set true for bench testing to stream full internal state over
// Serial (115200 baud): distances, closing velocities, TTEs,
// attitude, position, altitude, optical flow, map occupancy, and
// the fault byte. Set false for flight (compiles the block out —
// zero runtime cost). Prints every DEBUG_EVERY_N loops so it can't
// flood the 115200 link or distort loop-timing measurements.
static const bool DEBUG_SERIAL  = false;   // ← flip to true on the bench
#define DEBUG_EVERY_N 20   // print every 20th loop (~10 Hz at a 200 Hz loop)
...
// [v2.0.5 S-025] OF scale calibration — two-stage (fixes v2.0.4 bug where
// accumulation was reset AND started at the moment the distance was entered,
// i.e. AFTER the calibration flight had already happened, instead of
// accumulating DURING it).
// g_ofCalActive:   true from "go" until the distance is entered — of_update()
//                  accumulates world-frame displacement while this is true.
// g_ofCalFinalize: true once the distance has been entered — tells
//                  ble_update_notify() to compute the result from whatever
//                  was accumulated while g_ofCalActive was true, then reset.
static volatile bool g_ofCalActive   = false;
static volatile bool g_ofCalFinalize = false;
static float g_ofCalDistCm = 0.0f;
static float g_ofCalAccumX = 0.0f;
static float g_ofCalAccumY = 0.0f;
...
// Manual prototypes — prevents Arduino's auto-prototype generator from
// inserting these before MapPoint is defined
...
static void of_update(float dt, float altM,
  ...
  // Runs for the whole "go" → fly → enter-distance window (g_ofCalActive),
  // NOT just after the distance is entered — see calibration state comment
  // near g_ofCalActive's declaration.
  if (g_ofCalActive) {
    g_ofCalAccumX += wx * dt;
    g_ofCalAccumY += wy * dt;
  }
...
class BLEWriteCallback : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *c) override {
    ...
    if (uuid == BLE_CHAR_TTL_UUID) {
      ...
      // Two-stage: "go" arms accumulation BEFORE you fly; the distance is
      // entered AFTER flying, and finalizes using what was accumulated
      // during the actual flight (not after it — see v2.0.4 bug note above).
      if (sval.equalsIgnoreCase("go")) {
        g_ofCalAccumX = g_ofCalAccumY = 0.0f;
        g_ofCalActive   = true;
        g_ofCalFinalize = false;
        c->setValue("collecting - fly a known distance now, then write that distance in cm");
        ble_cal_notify("OF cal: fly now, then write distance flown in cm");
      } else if (!g_ofCalActive) {
        c->setValue("error: write 'go' first, then fly, then write distance in cm");
      } else {
        float d = sval.toFloat();
        if (d > 10.0f && d < 2000.0f) {
          g_ofCalDistCm   = d;
          g_ofCalActive   = false;   // stop accumulating — flight is done
          g_ofCalFinalize = true;    // ble_update_notify computes the result
          c->setValue("calculating...");
          ble_cal_notify("OF scale calculating");
        } else c->setValue("error: enter distance flown in cm (10-2000)");
      }
    }
...
static void ble_init() {
  ...
  bleCharOfCal->setValue("write 'go', fly a known distance, then write that distance in cm");
...
static void ble_update_notify(uint8_t faultByte) {
  ...
  // OF scale calibration result — fires once, after distance is entered,
  // using displacement accumulated DURING the flight (g_ofCalActive window).
  if (g_ofCalFinalize && bleCharOfCal) {
    g_ofCalFinalize = false;
...
void setup() {
  Serial.begin(115200);
  delay(300);
  Serial.println("[BOOT] Sensory Cortex v2.0.5");
...
void loop() {
  ...
  // ── 11. Debug output ── [bench] gated by DEBUG_SERIAL flag at top of file.
  // Prints ~10 Hz (every DEBUG_EVERY_N loops) so serial I/O can't stall the
  // loop or skew timing tests. Whole block is compiled out when DEBUG_SERIAL
  // is false (const-folded), so there is zero cost in a flight build.
  static uint16_t dbgN = 0;
  if (DEBUG_SERIAL && (++dbgN >= DEBUG_EVERY_N)) {
    dbgN = 0;
