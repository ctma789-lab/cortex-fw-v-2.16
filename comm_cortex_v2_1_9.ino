...
// [v2.1.9 I-105] The debug port and the RC input UART are now routed
...
//     iBUS RX (RC_SERIAL) → UART0. Set DEBUG_SERIAL true and watch
//     the decision stream over USB while feeding iBUS on GPIO4.
//
...
// begin(baud, config, rx, tx) overload for the iBUS init).
#if ARDUINO_USB_CDC_ON_BOOT
  #define USBSerial  Serial    // USB CDC — bench debug output
  #define RC_SERIAL  Serial0   // hardware UART0 — iBUS input
#else
  #define USBSerial  Serial    // UART0 (TX unattached — prints vanish)
  #define RC_SERIAL  Serial    // same UART0 — iBUS input
#endif
...
#define DBG_EVERY_N 15   // ~10 Hz at the ~142 Hz iBUS logic rate
...
static uint16_t g_failsafeRTH = 1000;   // failsafe PPM on CH_FLIGHT_MODE slot (RTH=1000); tunable via BLE
...
static void prefs_save() {
  ...
  g_prefs.putUShort("wdgIbus",   g_wdgIbusMs);
  g_prefs.end();
}
...
static void prefs_load() {
  ...
  g_wdgIbusMs          = g_prefs.getUShort("wdgIbus",  g_wdgIbusMs);
...
void setup() {
  ...
  delay(200);
  USBSerial.println("[BOOT] Communication Cortex v2.1.9");
  ...
  RC_SERIAL.begin(BAUD_IBUS, SERIAL_8N1, PIN_IBUS_RX, -1);  // RX only
  ...
  // ── BLE ───────────────────────────────────────────────────
  ble_cc_init();
  USBSerial.println("[INFO] Communication Cortex v2.1.9 running");
